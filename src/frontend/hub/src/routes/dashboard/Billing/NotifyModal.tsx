...
export default function NotifyModal(props: NotifyModalProps) {
  return (
    <Show when={props.notifyTier()}>
      <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-60 p-4" onClick={(e) => { if (e.target === e.currentTarget) props.setNotifyTier(null); }}>
        <div class="bg-xcord-bg-primary rounded-xl w-full max-w-sm p-6">
          ...
          <Show when={props.notifyStatus() !== 'success'} fallback={<p class="text-sm text-xcord-green py-4 text-center">{props.notifyMessage()}</p>}>
            ...
            <form onSubmit={props.onSubmit} class="space-y-3">
              ...
              <button type="submit" disabled={props.notifyStatus() === 'loading'} class="w-full py-2 bg-xcord-brand hover:bg-xcord-brand-hover disabled:opacity-50 text-white rounded font-medium transition text-sm">
